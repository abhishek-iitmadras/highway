...
// Safe reference implementation for floor division
template <typename T>
T SafeFloorDivScalar(T a, T b) {
  if constexpr (!hwy::IsSigned<T>()) {
    return static_cast<T>(a / b);
  } else {
    // Avoid scalar UB for INT_MIN / -1
    if (b == T(-1) && a == std::numeric_limits<T>::min()) {
      return T();
    }
    const T q = static_cast<T>(a / b);  // truncated
...
struct TestBasicDivision {
  ...
  void operator()(D d, size_t count, size_t misalign_a, size_t /*misalign_b*/,
    ...
    for (T divisor : divisors) {
      ...
      // Test with various dividend patterns
...
// ============================================================================
// Signed Division Tests (Truncating)
// ============================================================================
  ...
  void operator()(D d, size_t count, size_t misalign_a, size_t /*misalign_b*/,
    ...
    for (T divisor : divisors) {
      ...
      for (size_t i = 0; i < count; ++i) {
        ...
        // Skip INT_MIN / -1 case (scalar UB)
        if (divisor == T(-1) && a[i] == std::numeric_limits<T>::min()) {
          expected[i] = T();
        } else {
          expected[i] = static_cast<T>(a[i] / divisor);  // truncated division
        }
      }
      ...
      for (size_t i = 0; i < count; ++i) {
        // Skip verification for INT_MIN / -1
        if (divisor == T(-1) && a[i] == std::numeric_limits<T>::min()) {
          continue;
        }
        HWY_ASSERT_EQ(expected[i], actual[i]);
...
// ============================================================================
// Floor Division Tests (Python/NumPy semantics)
// ============================================================================
...
struct TestEdgeCases {
  ...
  void operator()(D d, size_t /*count*/, size_t /*misalign_a*/,
    ...
    // Signed-specific edge cases
    if constexpr (hwy::IsSigned<T>()) {
      const T kMin = std::numeric_limits<T>::min();
      const T kMax = std::numeric_limits<T>::max();
      
      // INT_MIN / -1 saturates to INT_MIN (avoids scalar UB)
      {
        const auto params = ComputeDivisorParams(T(-1));
...
struct TestConvenienceAPI {
  ...
  void operator()(D d, size_t count, size_t misalign_a, size_t /*misalign_b*/,
    ...
    // Test FloorDivideByScalar (signed only)
    if constexpr (hwy::IsSigned<T>()) {
      for (size_t i = 0; i < count; ++i) {
        expected[i] = SafeFloorDivScalar(a[i], divisor);
      }
...
struct TestArrayOperations {
  ...
  void operator()(D /*d*/, size_t /*count*/, size_t /*misalign_a*/,
    ...
    // Test FloorDivideArrayByScalar (signed only)
    if constexpr (hwy::IsSigned<T>()) {
